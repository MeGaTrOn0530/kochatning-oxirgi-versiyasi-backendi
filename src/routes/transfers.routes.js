...
import { ensureVarietyStockTable, adjustVarietyStock } from "../utils/greenhouse-stock.js";
      ...
      try {
        ...
        if (toLocRow?.type === 'greenhouse') {
          ...
          await adjustVarietyStock(
            conn, transfer.to_location_id, targetStage,
            bVarietyId, bSeedlingTypeId, bRootstockTypeId, transfer.quantity
          );
        ...
        if (transfer.receiver_confirmed || transfer.receiver_confirmed_by) {
          ...
          if (destInventory) {
            ...
            if (toLocRow?.type === 'greenhouse') {
              ...
              // receiver-confirm bosqichida qo'shilgan nav-stokni ham teskari qaytarish
              const [batchRows] = await conn.query(
                `SELECT variety_id, seedling_type_id, rootstock_type_id FROM seedling_batches WHERE id = ? LIMIT 1`,
                [transfer.batch_id]
              );
              const b = batchRows[0] || {};
              await adjustVarietyStock(
                conn, transfer.to_location_id, transfer.stage_on_transfer || 'cassette',
                b.variety_id || 0, b.seedling_type_id || 0, b.rootstock_type_id || 0, -transfer.quantity
              );
            }
          }
        }
